...
describe('button execute a function on click and modify his style class', () => {
    it('execute a function on click', () => {
        ...
        //BtnPlus rendering
        act(() => {
            render(<BurgerCenterToCross onClick={ onClick } isOpen={false} id={'1'} />, container);
        });
        //
    ...
    it('modify the css class of the button clicked', async () => {
        ...
        render(<BurgerCenterToCross onClick={ () => { isOpen=!isOpen } } isOpen={ isOpen } id={'1'}/>, container)
        //get the css classes of the button to be clicked
        const button = document.getElementsByClassName('burgerCenterToCross')[0]
